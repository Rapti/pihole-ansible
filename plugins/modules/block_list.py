...
description:
    ...
    - Supports batch processing of multiple block list entries.
    - Maps group names to their corresponding IDs.
options:
    lists:
        description:
            - List of block list entries to manage.
        required: false
        type: list
        elements: dict
        suboptions:
    ...
    address:
        description:
            - URL of the blocklist. For backward compatibility.
            - If both 'lists' and 'address' are provided, 'lists' takes precedence.
        required: false
        type: str
    state:
        description:
            - Whether the block list should be present or absent. For backward compatibility.
        required: false
        type: str
        choices: ['present', 'absent']
    comment:
        description:
            - Optional comment for the block list. For backward compatibility.
        required: false
        type: str
        default: null
    groups:
    ...
    enabled:
        description:
            - Whether the block list is enabled. For backward compatibility.
        required: false
        type: bool
        default: true
    password:
...
- name: Manage multiple block lists
  sbarbett.pihole.block_list:
    lists:
      ...
      - address: "https://s3.amazonaws.com/lists.disconnect.me/simple_tracking.txt"
        state: present
        comment: "Disconnect.me Tracking"
        groups:
          - Default
...
- name: Update a block list with group IDs (legacy)
  sbarbett.pihole.block_list:
    address: "https://raw.githubusercontent.com/StevenBlack/hosts/master/hosts"
    state: present
...
def run_module():
    ...
    try:
        ...
        module.fail_json(msg=f"Error managing block lists: {to_native(e)}", **result)
    finally:
